<div class="modal fade text-left" id="mdl_saving_pop" data-bs-backdrop="static" tabindex="-1" role="dialog"
    aria-labelledby="mdl_saving_pop_label" aria-hidden="true" wire:ignore.self>
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title" id="mdl_saving_pop_label">
                    @if ($is_edit)
                        Edit
                    @else
                        Tambah
                    @endif
                    Data Tabungan - {{ $title_saving }}
            ...
            <form>
                ...
                <div class="modal-footer">
                    ...
                    <button type="button" class="btn btn-primary"
                        wire:click="$dispatch('popSaving-save')">Simpan</button>
                </div>
            </form>
...
@push('scripts')
    ...
    <script>
        var mdl_saving_pop_elem = document.querySelector('#mdl_saving_pop');
        var mdl_saving_pop = null;

        document.addEventListener('livewire:initialized', () => {
            mdl_saving_pop = bootstrap.Modal.getOrCreateInstance(mdl_saving_pop_elem)

            // Listen to dispatch from server
            @this.on('popSaving_modal', (res) => {
                mdl_saving_pop.show();
                //alert('product created/updated')
                // var mdl_saving_pop_elem = document.querySelector('#mdl_saving_pop')
                // var modal = bootstrap.Modal.getOrCreateInstance(mdl_saving_pop_elem)
            ...
            mdl_saving_pop_elem.addEventListener('shown.bs.modal', (event) => {
            ...
            mdl_saving_pop_elem.addEventListener('hidden.bs.modal', (event) => {
                @this.dispatch('reset-modal');
                // $('#field_remider').prop('hidden', true);
            })
        ...
        document.addEventListener('livewire:navigated', () => {
            ...
            Livewire.on('popSaving_modal', (res) => {
                mdl_saving_pop.show();
                //alert('product created/updated')
                // var mdl_saving_pop_elem = document.querySelector('#mdl_saving_pop')
                // var modal = bootstrap.Modal.getOrCreateInstance(mdl_saving_pop_elem)
